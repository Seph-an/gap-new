...
export async function mailAgent({ name, email, phone, role, message }) {
  ...
  try {
    const clientHTML = render(Inquirer({ name, role }));
    const gapHTML = render(Inquiry({ name, email, phone, message }));

    // Send email to the client
    await sendEmail({
      sender: InquiryMail,
      receiver: email,
      subject: `Thank You for Your Inquiry ${name} - Gap Recruitment`,
      html: clientHTML,
    });
