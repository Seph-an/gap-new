...
export async function GET(request, { params }) {
  ...
  try {
    const response = await fetch(`${ERP_URL}/api/v1/cv-orders/${encodeURIComponent(reference)}`, { cache: 'no-store', headers: { authorization: `Bearer ${ERP_TOKEN}`, accept: 'application/json', 'x-status-token': token || '' } });
