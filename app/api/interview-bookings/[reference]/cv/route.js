...
export async function POST(request, { params }) {
  ...
  try {
    const response = await fetch(`${ERP_URL}/api/v1/interview-bookings/${encodeURIComponent(reference)}/cv`, {
      method: 'POST', body: request.body, duplex: 'half', cache: 'no-store',
      headers: { authorization: `Bearer ${ERP_TOKEN}`, 'content-type': request.headers.get('content-type') || 'application/octet-stream', accept: 'application/json' },
    });
